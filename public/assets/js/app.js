...
function renderChart(labels, values, containerId, height = 200, color = '#22c55e') {
  // Safe element selector (tumatanggap ng 'id', '#id', o DOM element)
  let el = null;
  if (typeof containerId === 'string') {
    const cleanId = containerId.startsWith('#') ? containerId.slice(1) : containerId;
  ...
  // Validation para sa empty o invalid data arrays
  labels = Array.isArray(labels) && labels.length ? labels : ['00:00'];
  values = Array.isArray(values) && values.length ? values.map(v => Number(v) || 0) : [0];

  // Kapag 1 lang ang data point, lagyan ng zero baseline para mag-render pa rin ang linya
  if (values.length === 1) {
    values = [0, values[0]];
    labels = ['', labels[0]];
  }
  ...
  else if (rawMax <= 500) yMax = 500;
  else yMax = Math.ceil(rawMax / 500) * 500;

  const ySteps = [yMax, yMax * 0.75, yMax * 0.5, yMax * 0.25, 0].map(v => Math.round(v));

  // Compute SVG Points safely
  const pts = values.map((v, i) => {
    const x = (i / (values.length - 1)) * W;
    const y = H - 10 - (Math.min(v, yMax) / yMax) * (H - 20);
    return `${x.toFixed(1)},${y.toFixed(1)}`;
  }).join(' ');

  const step = Math.max(1, Math.ceil(labels.length / 8));
  const xLabs = labels.filter((_, i) => i % step === 0).map(l => `<span>${l}</span>`).join('');
  const gradId = 'cg_' + Math.random().toString(36).substring(2, 7);

  el.innerHTML = `
    <p class="chart-y-label">Power (W)</p>
    <div class="chart-wrap">
      <div class="chart-y">${ySteps.map(v => `<span>${v >= 1000 ? (v / 1000).toFixed(1) + 'k' : v}</span>`).join('')}</div>
...
// ── Dashboard Loader (Walang demoChart) ──────────────────────────
async function loadDashboardData() {
  try {
    const res = await fetch('api.php?action=dashboard_stats').then(r => r.json());

    if (res.status === 'ok' || res.status === 'success') {
      const data = res.data || res;

      // 1. I-render ang Live Chart Data mula sa Backend API
      if (data.chart && Array.isArray(data.chart.values) && data.chart.values.length > 0) {
        renderChart(data.chart.labels, data.chart.values, '#dashboard-chart');
      } else {
        // Fallback kung blangko ang database ngayong araw (Zero Line)
        const currentTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        renderChart([currentTime], [0], '#dashboard-chart');
      }
...
// ── Helper: Room Cards Generator ─────────────────────────────
function buildRoomCardsHTML(rooms) {
  if (!rooms || !rooms.length) {
    return `<p class="text-muted" style="font-size:13px;padding:20px 0">No rooms yet. Add rooms and connect the ESP32.</p>`;
  }
  return rooms.map(r => {
    const isA = r.status === 'anomaly';
    const ri = ROOM_ICONS[r.icon_key || 'other'] || ROOM_ICONS.other;
    const powerVal = parseFloat(r.power_watts || 0).toFixed(0);
    return `<div class="room-card ${isA ? 'anomaly' : ''}">
      <div class="room-card-icon" style="background:${ri.bg}">${ri.svg}</div>
      <h4>${r.room_name}</h4>
      <p>${r.equipment_label}</p>
      <div class="room-power ${isA ? 'bad' : 'ok'}">${powerVal} W</div>
...
async function renderDashboard() {
  ...
  let stats = null;
  try { 
    stats = await api('dashboard_stats'); 
  } catch (err) {
  ...
  const todayE = parseFloat(stats?.today_energy ?? stats?.today_kwh ?? 0).toFixed(2);
  const monthE = parseFloat(stats?.month_energy ?? stats?.month_kwh ?? 0).toFixed(2);
  const activeAnom = parseInt(stats?.active_anomalies ?? rooms.filter(r => r.status === 'anomaly').length, 10);
  const now = new Date();
  ...
  pc.innerHTML = `
    <div class="stats-row">
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-green" id="total-power">${totalPower.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} W</h2>
          <small id="power-updated-time">Updated: ${now.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</small>
        </div>
      </div>
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-blue" id="today-energy">${todayE} kWh</h2>
      ...
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-yellow" id="month-energy">${monthE} kWh</h2>
      ...
      <div class="stat-card">
        ...
        <div class="stat-text">
          ...
          <h2 class="text-red" id="active-anomalies">${activeAnom}</h2>
    ...
    <div class="dash-bot">
      <div class="room-section">
        ...
        <div class="room-scroll" id="room-cards">${buildRoomCardsHTML(rooms)}</div>
      </div>
  ...
  const cLabels = stats?.chart?.labels || stats?.chart_labels || (typeof demoChart === 'function' ? demoChart().labels : []);
  const cValues = stats?.chart?.values || stats?.chart_values || (typeof demoChart === 'function' ? demoChart().values : []);
  if (document.getElementById('main-chart')) {
    renderChart(cLabels, cValues, 'main-chart', 230);
  }
...
// ── Dashboard Live Update Function ───────────────────────────
async function updateDashboardLive() {
  try {
    const stats = await api('dashboard_stats');
    if (!stats) return;

    if (stats.rooms) state.rooms = stats.rooms;
    const currentRooms = state.rooms || [];

    // 1. Live Sum ng Total Power mula sa active rooms
    const liveTotalWatts = currentRooms.reduce((s, r) => s + parseFloat(r.power_watts || 0), 0);
    
    // 2. Pagkuha ng energy values at active anomalies count
    const todayKwh = parseFloat(stats.today_energy ?? stats.today_kwh ?? 0);
    const monthKwh = parseFloat(stats.month_energy ?? stats.month_kwh ?? 0);
    const activeAnomCount = parseInt(stats.active_anomalies ?? currentRooms.filter(r => r.status === 'anomaly').length, 10);

    // 3. Live Update sa Top Cards Text
    const totalPowerEl = document.querySelector('#total-power');
    const todayEnergyEl = document.querySelector('#today-energy');
    const monthEnergyEl = document.querySelector('#month-energy');
    const anomaliesEl = document.querySelector('#active-anomalies');
    const timeEl = document.querySelector('#power-updated-time');

    if (totalPowerEl) totalPowerEl.innerText = `${liveTotalWatts.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ',')} W`;
    if (todayEnergyEl) todayEnergyEl.innerText = `${todayKwh.toFixed(2)} kWh`;
    if (monthEnergyEl) monthEnergyEl.innerText = `${monthKwh.toFixed(2)} kWh`;
    if (anomaliesEl) anomaliesEl.innerText = activeAnomCount;
    if (timeEl) timeEl.innerText = `Updated: ${new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}`;

    // 4. Update Header & Navigation Badges
    const bellBadge = document.querySelector('#bell-badge');
    const navBadge = document.querySelector('#nav-anom-badge');
    if (bellBadge) { 
      bellBadge.textContent = activeAnomCount; 
      bellBadge.classList.toggle('hidden', activeAnomCount === 0); 
    }
    if (navBadge) { 
      navBadge.textContent = activeAnomCount; 
      navBadge.classList.toggle('hidden', activeAnomCount === 0); 
    }

    // 5. Update Room Cards sa ibaba
    const rcEl = document.querySelector('#room-cards');
    if (rcEl) {
      rcEl.innerHTML = buildRoomCardsHTML(currentRooms);
    }

    // 6. Live Update sa Power Chart
    const cLabels = stats?.chart?.labels || stats?.chart_labels;
    const cValues = stats?.chart?.values || stats?.chart_values;
    if (cLabels && cValues && document.getElementById('main-chart')) {
      renderChart(cLabels, cValues, 'main-chart', 230);
    }
...
// ==========================================
// LIVE AUTO-POLLING (Kada 3 segundo)
// ==========================================
setInterval(async () => {
  // 1. Kung nasa Dashboard page
  if (state.page === 'dashboard') {
    await updateDashboardLive();
  }

  // 2. Kung nasa Real-time Monitoring page
  if (state.page === 'monitoring') {
    checkEsp32Status();

    const activeItem = document.querySelector('.monitor-list-item.active');
    if (activeItem) {
      const activeRoomId = activeItem.dataset.rid;
      try {
        const s = await api('dashboard_stats');
        state.rooms = s.rooms || [];
        
        const activeRoom = state.rooms.find(r => r.room_id == activeRoomId);
        if (activeRoom) {
          selectMonitor(activeRoomId);
        }
      } catch (e) {
        console.log('Error refreshing monitoring:', e);
      }
    }
  }
}, 3000);